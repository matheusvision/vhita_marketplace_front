...
const messengerApi = [
	 ...
	 */
	http.get('/api/mock/messenger/contacts', async ({ params, request }) => {
	 ...
	 */
	http.get('/api/mock/messenger/contacts/:id', async ({ params, request }) => {
		const api = mockApi('messenger_contacts');
	 ...
	 */
	http.delete('/api/mock/messenger/contacts/:id', async ({ params, request }) => {
		const api = mockApi('messenger_contacts');
		const { id } = params as Record<string, string>;
	 ...
	 */
	http.get('/api/mock/messenger/chat-list', async ({ params, request }) => {
	 ...
	 */
	http.post('/api/mock/messenger/chat-list', async ({ params, request }) => {
		const api = mockApi('messenger_chat_list');
		const data = (await request.json()) as Record<string, unknown>;
	 ...
	 */
	http.get('/api/mock/messenger/messages', async ({ params, request }) => {
	 ...
	 */
	http.post('/api/mock/messenger/messages', async ({ params, request }) => {
		const api = mockApi('messenger_messages');
		const data = (await request.json()) as Record<string, unknown>;
		const newItem = await api.create(data);
		return HttpResponse.json(newItem, { status: 201 });
	 ...
	 */
	http.get('/api/mock/messenger/profiles', async ({ params, request }) => {
	 ...
	 */
	http.get('/api/mock/messenger/profiles/:id', async ({ params, request }) => {
		const api = mockApi('messenger_profiles');
		const { id } = params as Record<string, string>;
		const item = await api.find(id);
